...
class KDD_DATASET(Dataset):
    def __init__(self, configs):
        ...
        self.data = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/KDD_norm.csv", delimiter=",")[0:8016].reshape(-1, self.configs.seq_len, self.configs.enc_in)
        if self.configs.missing_rate == 0:
            self.mask = np.ones_like(self.data)
            self.mask[np.where(np.isnan(self.data))] = 0
        else:
            self.mask = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/data/kdd_"+str(self.configs.missing_rate)+ "_"  + str(self.configs.seed) +  ".csv", delimiter=",")[0:8016].reshape(-1, self.configs.seq_len, self.configs.enc_in)
        self.mask_gt = np.ones_like(self.data)
        self.mask_gt[np.where(np.isnan(self.data))] = 0
...
class GUANGZHOU_DATASET(Dataset):
    def __init__(self, configs):
        ...
        self.configs = configs
        self.data = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/Guangzhou_norm.csv", delimiter=",")[0: 8784 // 48 * 48].reshape(-1, self.configs.seq_len, self.configs.enc_in)
        self.mask = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/data/guangzhou_"+str(self.configs.missing_rate)+ "_"  + str(self.configs.seed) +  ".csv", delimiter=",")[0: 8784 // 48 * 48].reshape(-1, self.configs.seq_len, self.configs.enc_in)
        self.mask_gt = np.ones_like(self.data)
        self.mask_gt[np.where(self.data == -200)] = 0
        self.data[self.data == -200] = 0.0
        self.dataf = np.array(self.data)
        self.dataf[np.where(self.mask == 0)] = np.nan
        self.dataf = torch.from_numpy(self.dataf).float()
        self.dataf = torchcde.linear_interpolation_coeffs(self.dataf)
...
class PHYSIO_DATASET(Dataset):
    def __init__(self, configs):
        ...
        self.configs = configs
        self.data = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/Physio_norm.csv", delimiter=",").reshape(-1, self.configs.seq_len, self.configs.enc_in)
        self.mask = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/data/physio_"+str(self.configs.missing_rate)+ "_"  + str(self.configs.seed) +  ".csv", delimiter=",").reshape(-1, self.configs.seq_len, self.configs.enc_in)
        self.mask_gt = np.ones_like(self.data)
        self.mask_gt[np.where(self.data == -200)] = 0
        self.data[self.data == -200] = 0.0
        self.dataf = np.array(self.data)
