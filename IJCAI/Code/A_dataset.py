...
class KDD_DATASET(Dataset):
    def __init__(self, configs):
        ...
        if self.configs.missing_rate == 0:
            ...
            #针对MCAR，MNAR，MAR这里要修改 (for MCAR and MNAR and MAR you need to modify here)
        ...
        if hasattr(self.configs, 'flimit'):
            temp_list = []
...
class GUANGZHOU_DATASET(Dataset):
    def __init__(self, configs):
        ...
        self.mask = np.loadtxt("/mnt/4T/IJCAI/FGTI/Data/data/guangzhoumnar_"+str(self.configs.missing_rate)+ "_"  + str(self.configs.seed) +  ".csv", delimiter=",")[0: 8784 // 48 * 48].reshape(-1, self.configs.seq_len, self.configs.enc_in)
        self.mask_gt = np.ones_like(self.data)
